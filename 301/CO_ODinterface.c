...
OD_readOriginal(OD_stream_t* stream, void* buf, OD_size_t count, OD_size_t* countRead) {
    ...
    if((stream->attribute & ODA_STR) == 0) {
        if(stream->dataLength <= 2) {
            ...
            (void)memcpy((void*)buf, (const void*)dataOrig, 1);
        } else {
            (void)memcpy((void*)buf, (const void*)dataOrig, dataLenToCopy / 2);
        }
    } else {
        /* As-is for string */
...
OD_writeOriginal(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten) {
    ...
    if ((dataLenToCopy <= dataLenRemain) && (dataLenToCopy <= count)) {
        ...
        if((stream->attribute & ODA_STR) == 0) {
            if(stream->dataLength <= 2) {
                /* For C2000, length of uint8_t and uint16_t OD variable is 1 */
                (void)memcpy((void*)dataOrig, (const void*)buf, 1);
            } else {
                (void)memcpy((void*)dataOrig, (const void*)buf, dataLenToCopy / 2);
            }
        } else {
            /* As-is for string */
