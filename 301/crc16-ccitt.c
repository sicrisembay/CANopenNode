...
#if (C2000_PORT != 0)
    uint8_t tmp = (uint8_t)((((*crc >> 8U) & 0x00FF) ^ chr) & 0x00FF);
#else
...
#if (C2000_PORT != 0)
uint16_t
crc16_ccitt(uint8_t * block, size_t blockLength, uint16_t crc) {
#else
uint16_t
...
#if (C2000_PORT != 0)
        uint8_t tmp = (uint8_t)((((uint8_t)((crc >> 8U) & 0x00FF) ^ block[i])) & 0x00FF);
#else
